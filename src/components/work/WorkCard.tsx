 ...
 * image at its native aspect ratio, and a caption rail that reveals the brief
 * on hover. Native aspect preserved so the wall reads like curation, not a grid.
 */
export function WorkCard({
  ...
  return (
    <figure
      className={clsx(
        "group relative mb-6 break-inside-avoid overflow-hidden rounded-2xl border border-line bg-ink-3 p-2.5 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-1 hover:border-gold-line hover:shadow-[var(--shadow-lift)]",
        className
      )}
    >
